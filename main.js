...
function updateCameraSelector(deviceInfos) {
  ...
  for (let i = 0; i !== deviceInfos.length; ++i) {
    ...
    if (deviceInfo.kind === "videoinput") {
      ...
      option.text =
        deviceInfos[i].label || `camera ${select_camera.length + 1}`;
...
function onCameraError(err) {
  ...
  alert(
    `カメラ映像を読み込めませんでした。ブラウザのアクセス制限など，設定を確認してください`
  );
...
async function translateToEng(string) {
  const res = await fetch(`https://api-free.deepl.com/v2/translate`, {
    method: "POST", // *GET, POST, PUT, DELETE, etc.
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
    },
    body: `auth_key=ab5974c7-caa8-ba6d-3fac-0ec46ea54a9b&source_lang=JA&text=${string}&target_lang=EN-US`,
  });
  ...
  recognition.onresult = function (event) {
    ...
    for (var i = event.resultIndex; i < results.length; i++) {
      if (results[i].isFinal) {
        ...
        translateToEng(result_transcript).then((result) => {
...
function downloadLogFile() {
  ...
  a.href =
    "data:text/plain," +
    encodeURIComponent(document.getElementById("result_log").value);
...
var font_size_windowed = parseFloat(
  getComputedStyle(objResultText).getPropertyValue("font-size")
);
  ...
  eventFullScreen(() => {
    ...
    if (getFullScreenObject()) {
      ...
      const ratio =
        ...
        document.querySelector("#result_video").clientHeight;
      font_size_windowed = parseFloat(
        getComputedStyle(objResultText).getPropertyValue("font-size")
      );
      if (objResultText.style.fontSize) {
        ...
        font_size_windowed = parseFloat(
          getComputedStyle(objResultText).fontSize
        );
      }
      ...
      if (flag_font_size_styled) {
        document.querySelector("#result_text").style.fontSize =
          document.querySelector("#value_font_size").textContent + "px";
      } else {
...
var fonts_custom = [
  ...
  [
    "BIZ UD ゴシック（Windows 10）",
    "'BIZ UDゴシック', 'BIZ UDGothic', 'Noto Sans JP', sans-serif",
  ],
  [
    "BIZ UD 明朝（Windows 10）",
    "'BIZ UD明朝', 'BIZ UDMincho', 'Noto Sans JP', sans-serif",
  ],
...
function initConfig() {
  ...
  ["video_bg", "result_video", "text_overlay_wrapper", "FullScreenBtn"].forEach(
    (id) => {
      if (typeof config[id] !== "undefined") {
        const el = document.getElementById(id);
        if (config[id]) {
  ...
  document
    .querySelectorAll("input.control_input")
    .forEach((el) => el.addEventListener("input", updateConfigValue));
  document
    .querySelectorAll('input[name="selector_position"]')
    .forEach((el) =>
      el.addEventListener("input", (ev) => updateConfig("position", el.id))
    );
  document
    .querySelector("#select_camera")
    .addEventListener("change", updateConfigValue);
  document
    .querySelector("#select_font")
    .addEventListener("change", updateConfigValue);
  document
    .querySelector("#select_autoclear_text")
    .addEventListener("change", updateConfigValue);
}
...
function initKuromoji(checkbox) {
  if (checkbox.checked == true && kuromojiObj == undefined) {
    document.getElementById("status_kuromoji_loading").innerHTML =
      "ひらがなデータ読み込み中...";
